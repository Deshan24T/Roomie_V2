...
<head>
    ...
    <title>Add Room – Roomie Admin</title>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .upload-area{border:2px dashed var(--border);border-radius:14px;padding:1.5rem;text-align:center;cursor:pointer;transition:all .3s;position:relative;}
        ...
        .upload-label{font-size:.85rem;color:var(--text-muted);}
        .upload-label span{color:var(--gold);font-weight:600;}
        #imagePreview{display:none;margin-top:1rem;width:100%;max-height:180px;object-fit:cover;border-radius:12px;border:1px solid var(--border);}
...
<div class="container">
    ...
    <div class="form-card">
        <form action="/room/add" method="post" enctype="multipart/form-data">
            <div class="form-row">
                <div class="form-group"><label for="roomNumber">Room Number</label><input type="text" id="roomNumber" name="roomNumber" placeholder="e.g. 302" required></div>
                <div class="form-group"><label for="type">Room Type</label><select id="type" name="type" required><option value="Standard">Coastal (Standard)</option><option value="Deluxe">Hill Country (Deluxe)</option><option value="Suite">Jungle (Suite)</option></select></div>
            </div>
            <div class="form-row">
                <div class="form-group"><label for="pricePerNight">Price Per Night (LKR)</label><input type="number" id="pricePerNight" name="pricePerNight" placeholder="5500" required min="1000"></div>
                <div class="form-group"><label for="capacity">Capacity (Guests)</label><input type="number" id="capacity" name="capacity" placeholder="2" required min="1" max="10"></div>
            </div>
            <div class="form-group"><label for="description">View Description</label><input type="text" id="description" name="description" placeholder="Ocean / Garden / Pool / Panoramic" required></div>
            <div class="form-group"><label for="amenities">Amenities</label><input type="text" id="amenities" name="amenities" placeholder="WiFi|AC|TV|Balcony" required><div class="hint">Separate with | (pipe) character. E.g. WiFi|AC|TV|Mini Bar</div></div>
            <div class="form-group"><label>Room Image <span style="color:var(--text-muted);font-weight:400;">(optional)</span></label>
                <div class="upload-area" id="uploadArea"><input type="file" id="image" name="image" accept="image/*"><div class="upload-icon">🖼️</div><div class="upload-label">Drop an image here or <span>browse</span></div><div class="hint">JPG, PNG, WEBP · Max 10 MB</div><img id="imagePreview" src="" alt="Preview"><div id="selectedFileName"></div></div>
            </div>
